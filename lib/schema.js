...
const convertType = (field) => {
    ...
    if (!convType) {
        convType = mongoose.Schema.Types.Mixed;
    }
...
function convertSchema(jsonSchema) {
    ...
    delete jsonSchema.properties._id;  // https://github.com/Automattic/mongoose/issues/3638
    for (let key of Object.keys(props)) {
        var field = props[key];
        var entry = field.mongo || {};
        entry.required = required.indexOf(key) !== -1;
        if (field.type === 'object') {
            entry = convertSchema(field);
        } else {
            let validator = getValidator(field);
            entry.type = convertType(field);
